...
import { DraftLeaderboardPreview } from "./DraftLeaderboardPreview";
...
export const ExpansionDraft = () => {
  ...
  // Lazily-loaded rosters keyed by team abbr: { players, protection, ... }.
  const [rosters, setRosters] = useState({});
  const [activeTeam, setActiveTeam] = useState(null);
  const [rosterLoading, setRosterLoading] = useState(false);

  // picks: { [team]: pickedPlayer }
  const [picks, setPicks] = useState({});
  const [confirm, setConfirm] = useState(null); // { team, player }
  ...
  // Picks to reapply once a resumed season's teams have loaded.
  const [pendingResume, setPendingResume] = useState(null);
  const [confirmDelete, setConfirmDelete] = useState(null); // draft
  ...
  // Auto scroll to team carousel when teams load
  useEffect(() => {
    if (teams.length > 0) {
  ...
  // Auto scroll to roster table when active team's roster is loaded
  useEffect(() => {
  ...
  // Auto scroll the horizontal carousel to center the active team card
  useEffect(() => {
  ...
  // Horizontal franchise carousel.
  const carouselRef = useRef(null);
  const scrollCarousel = (dir) => {
    const el = carouselRef.current;
    if (!el) return;
    // Assigning scrollLeft (rather than scrollBy) animates via the container's
    // CSS `scroll-behavior: smooth` and behaves reliably with scroll-snap.
    el.scrollLeft += dir * el.clientWidth * 0.85;
  };

  // Prompt user when closing window/tab if draft is in progress
  useEffect(() => {
  ...
  // Reset everything when the season changes.
  useEffect(() => {
    if (!season) return;
    let cancelled = false;
  ...
  // Resume a saved in-progress draft: switch to its season and stash its picks
  // so they can be reapplied once that season's franchises have loaded (the
  // season-change effect above clears picks synchronously).
  ...
  const confirmDeleteSaved = async () => {
    ...
    try {
      if (d.leaderboardId) {
        await deletePostedDraft(d.leaderboardId);
      }
    } catch {
      // best-effort
    }
  ...
  // Reapply resumed picks once the matching season's teams are available.
  useEffect(() => {
  ...
  // Resume from a navigation (account menu "in progress" entry). Keyed on the
  // navigation so it also fires when already on this page.
  useEffect(() => {
  ...
  const loadRoster = async (team) => {
    ...
    try {
      ...
      setRosters((prev) => ({ ...prev, [team]: data }));
    } catch (err) {
      setRosters((prev) => ({ ...prev, [team]: { error: err.message } }));
    } finally {
  ...
  const confirmPick = () => {
    if (!confirm) return;
    setPicks((prev) => ({ ...prev, [confirm.team]: confirm.player }));
    setConfirm(null);
    // Auto-advance to the next undrafted team for momentum.
  ...
  const handleSaveProgressClick = () => {
    ...
    setSaveError("");
    // If we've already saved this draft before, overwrite directly.
  ...
  const confirmSaveProgress = async (allowFallbackName = false) => {
    ...
    try {
      await saveDraftProgress(user.uid, { season, picks, teamName: name });
      setSavedProgressHash(
        JSON.stringify({
          season: String(season || ""),
          teamName: name,
  ...
  const completeDraft = async () => {
    const pickList = teams.map((t) => ({ ...picks[t.team], team: t.team }));
    ...
    try {
      ...
      const draft = { season, teamName: name, profile };
      // Persist the finished franchise and retire the in-progress autosave. The
      // saved id tells the result page the draft is already in the account.
      if (user) {
        try {
          draft.savedDraftId = await saveCompletedDraft(user.uid, draft);
          await deleteDraftProgress(user.uid, season);
        } catch {
          /* saving to the account is best-effort; the result still renders */
        }
      }
      sessionStorage.setItem(DRAFT_STORAGE_KEY, JSON.stringify(draft));
      navigate("/expansion-draft/result", { state: { draft } });
    } catch (err) {
  ...
  return (
    <div className="min-h-screen ice-background px-4 pb-28 pt-5 text-white light:text-gray-900 sm:px-6 sm:py-8">
      <div className="mx-auto max-w-6xl">
        ...
        {/* Resume in-progress drafts saved to the account */}
        ...
        {teams.length > 0 && (
          <>
            ...
            >
              ...
              {/* Positional Requirements Progress */}
              <div className="mt-4 grid grid-cols-3 gap-3 rounded-2xl border border-white/5 bg-white/5 p-4 light:border-slate-200 light:bg-white/60">
                <div className="flex flex-col items-center justify-center text-center">
                  ...
                  <span
                    className={`mt-1 text-lg sm:text-2xl font-black tabular-nums ${
                      forwardCount >= 12
              ...
              {/* Warning banner when all teams drafted but positional requirements not met */}
              {draftedCount === teams.length &&
                !satisfiesPositionalRequirements && (
              ...
              <div ref={rosterSectionRef} className="mt-8 scroll-mt-20">
                {rosterLoading && !activeRoster ? (
                  ...
                  <RosterTable
                    ...
                    onPick={(player) =>
                      setConfirm({ team: activeTeam, player })
                    }
                  />
                ) : null}
              </div>
      ...
      {/* Floating Complete Draft button */}
      {teams.length > 0 && (
        <button
          type="button"
          disabled={!isComplete}
          onClick={() => setShowNameModal(true)}
          className={`fixed bottom-6 right-6 z-40 btn-search-primary btn-search-primary-inline shadow-2xl transition-all duration-300 !min-h-[50px] py-3 text-sm ${
            isComplete ? "hover:scale-105 active:scale-99" : ""
          }`}
        >
          {isComplete ? (
...
// --------------------------------------------------------------------------
// Team grid card — logo, name, drafted-player slot.
// --------------------------------------------------------------------------
const TeamCard = ({ team, season, actualTheme, pick, isActive, onClick }) => (
  <button
    type="button"
    onClick={onClick}
    className={`group relative flex h-full w-full flex-col items-center rounded-[22px] border p-3 text-center transition-all duration-200 ${
      isActive
        ? "border-sky-400/60 bg-sky-400/10"
        : "border-white/10 bg-white/5 hover:border-white/25 hover:bg-white/10 light:border-slate-200 light:bg-white/60"
    }`}
  >
    <img
...
const RosterTable = ({ roster, season, isMobile, pickedId, onPick }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Forwards and Defense inline on desktop (md), stacked on mobile */}
      ...
      {players.map((p) => {
        ...
        return (
          <tr
            ...
            className={`border-t border-white/5 light:border-slate-200 transition-colors ${
              locked || isExempt
                ? "opacity-40"
                : picked
                  ? "bg-emerald-500/10"
                  : "cursor-pointer hover:bg-white/5 light:hover:bg-slate-900/5"
            }`}
          >
...
const Cell = ({ children, strong }) => (
  <td
    className={`px-2 py-2 text-center tabular-nums ${
      strong
        ? "font-semibold text-white light:text-gray-900"
