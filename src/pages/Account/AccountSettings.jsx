...
//
// Home for the account actions that used to live inline in the header dropdown:
// email-verification status + resend, and the password-reset email. Reached via
// the "Account settings" item in the avatar menu (/account).
...
// letter. Mirrors the avatar logic in AccountMenu.
...
export const AccountSettings = () => {
  ...
  // Signed-out visitors get bounced to the home page with the auth modal open.
  useEffect(() => {
    if (!loading && !user) {
      openAuth("signin");
        ...
        >
          <div className="liquid-glass-strong relative w-full max-w-md rounded-[32px] p-6 sm:p-8">
            ...
            <button
              ...
              disabled={
                busy === "name" ||
                nameDraft.trim().replace(/\s+/g, " ") === (user.displayName || "")
              }
