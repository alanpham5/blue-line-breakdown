...
export const PlayerHeader = ({ player, biometrics }) => {
  const teamLogoUrl = player.team ? playerUtils.getTeamLogoUrl(player.team, player.season) : null;
  return (
    <div className="liquid-glass-strong rounded-2xl p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
        ...
        <div className="flex-1 min-w-0 text-center sm:text-left">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-2 sm:mb-3 truncate">{player.name}</h2>
          <div className="space-y-2">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 sm:gap-3 text-gray-300">
              <span className="text-sm sm:text-base">{playerUtils.formatSeason(player.season)}</span>
              <span className="text-gray-500">•</span>
              <span className="text-sm sm:text-base">{player.position === "F" ? "Forward" : "Defense"}</span>
            </div>
              ...
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 sm:gap-4">
                {biometrics?.height && (
                  <div className="flex items-center gap-1.5 text-gray-300">
                    ...
                    <span className="text-sm sm:text-base">{biometrics.height}</span>
                  </div>
                )}
                {biometrics?.weight && (
                  <div className="flex items-center gap-1.5 text-gray-300">
                    <Scale className="w-4 h-4 text-cyan-400" />
                    <span className="text-sm sm:text-base">{Math.round(biometrics.weight)} lbs</span>
                  </div>
                )}
              </div>
            )}
