// Authentication modal: sign in / sign up / password reset, plus the Google
// SSO account-linking flow. Single instance is owned by AuthProvider and opened
// via `openAuth()`. Vertical layout, liquid-glass styling to match the app.
...
const VIEW = {
  ...
  SIGNUP: "signup",
  RESET: "reset",
  LINK: "link", // Google credential pending — confirm original password
};
...
export const AuthModal = () => {
  ...
  // Reset state each time the modal opens, honoring the requested mode.
  useEffect(() => {
    if (isOpen) {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      if (err?.code === ACCOUNT_EXISTS_CODE) {
        // Drop into the linking view: keep the pending Google credential and
        // ask the user to confirm their original password.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === VIEW.SIGNIN) {
        ...
        // Stay open briefly so the user sees the verification notice.
  ...
  return (
    ...
    >
      <div className="liquid-glass-strong relative w-full max-w-md rounded-[32px] p-6 sm:p-8">
        ...
        {/* Google SSO — hidden in reset + link views */}
