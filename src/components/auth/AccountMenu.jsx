// Account avatar dropdown for the global header.
//
//  - Signed out: a compact "Sign in" button that opens the AuthModal.
//  - Signed in:  avatar (Google photo or initials) → dropdown with Account
//    (status + settings link), My Bookmarks and My Drafts (live from Firestore,
//    capped to the most recent few with a "see all" page link), Theme, and Sign
//    out.
import {
  useCallback,
  useEffect,
...
import { seasonSpan } from "../../pages/ExpansionDraft/draftShared";
import { bookmarkHref, draftNavTarget } from "../../pages/Account/savedItems";
import { ThemeToggle } from "../ThemeToggle";
import { ConfirmDialog } from "../ConfirmDialog";

// Most-recent items shown inline; the rest live on the dedicated "see all" page.
const MAX_INLINE = 3;

// Up to two initials from a display name, falling back to the email's first
// letter, then a generic glyph. Drives the avatar when there's no photoURL.
...
export const AccountMenu = () => {
  ...
  // Screen-space rect of the avatar button; positions the portalled menu.
  ...
  useEffect(() => {
    if (!open) return;
    // The menu is portalled to <body>, so the outside-click check must exempt
    // both the trigger and the menu itself.
  ...
  // While the initial auth state resolves, render a neutral placeholder so the
  ...
  const savedDrafts = drafts.filter(
    (d) =>
      isCompleteDraft(d) ||
      (isInProgressDraft(d) && (d.pickCount || 0) > 0)
  );
      ...
      {rect &&
        createPortal(
          <div
            ...
            className={`liquid-glass-strong z-[9999] w-[19rem] origin-top-right overflow-hidden rounded-[26px] p-2 transition-all duration-200 ease-out ${
              open
          ...
          >
            ...
            <div className="px-3 pb-3 pt-2">
              ...
              {/* Verification status badge (read-only; actions live on /account) */}
              <div className="mt-3">
                {user.emailVerified ? (
...
// A saved-draft row with an open action and an inline delete control.
