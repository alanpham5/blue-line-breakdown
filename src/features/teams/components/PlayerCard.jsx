...
export const PlayerCard = ({ player, team, season, stroke, onPlayerClick }) => {
  ...
  const winShare = player.winShare ?? 0;
  const gaugeWidth = Math.min(Math.abs(winShare), maxWinShare);
    ...
    >
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-sm">
          ...
          <span
            className={`${winShare >= 0 ? "text-cyan-300 light:text-cyan-700" : "text-rose-400 light:text-rose-700"} font-bold bg-white/[0.06] light:bg-gray-200/80 px-2 py-0.5 rounded-full`}
          >
            {winShare.toFixed(1)}
          </span>
        </div>
        <div className="w-full bg-white/[0.06] light:bg-gray-200 rounded-full h-2">
          <div
            className={`bg-gradient-to-r ${winShare >= 0 ? "from-cyan-500 to-sky-400" : "from-rose-500 to-red-400"} h-2 rounded-full transition-all duration-300 gauge-fill`}
            style={{
              width: `${gaugeWidth}%`,
            }}
          />
