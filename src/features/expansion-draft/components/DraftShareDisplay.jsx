        ...
        >
          <img
            src={playerUtils.getCorsWrappedUrl(
              playerUtils.getPlayerHeadshot(p.playerId, p.team, season)
            )}
            alt={p.name}
          ...
          <span className={`text-sm font-extrabold shrink-0 ${accent}`}>
            {p.warPercentile != null ? `${p.warPercentile.toFixed(1)}%` : "—"}
          </span>
        </div>
      ))}
    </div>
...
export const DraftShareDisplay = ({ draft, profile }) => {
  if (!profile) {
    return (
      <div
        style={{
  ...
  const { stats, predictedRecord, draftSummary, roster, similarTeams } =
    profile;
  const season = draft.season;
  const byImpact = (a, b) => (b.warPercentile ?? -1) - (a.warPercentile ?? -1);
  const topForwards = roster
    .filter((p) => p.position === "F")
    .sort(byImpact)
    .slice(0, 4);
  const topDefensemen = roster
    .filter((p) => p.position === "D")
    .sort(byImpact)
    .slice(0, 4);
  const topGoalies = roster
    .filter((p) => p.position === "G")
    .sort(byImpact)
    .slice(0, 2);
  const counts = draftSummary.counts;
  const playoffOdds =
    predictedRecord.playoffProbability != null
      ? Math.round(predictedRecord.playoffProbability * 100)
      : null;
  return (
    <div
    ...
    >
      ...
      <div
        style={{
          ...
          display: "grid",
          gridTemplateRows: "auto 1.45fr 0.85fr auto",
          gap: 10,
        }}
      >
        <div className="liquid-glass-strong rounded-[28px] overflow-hidden px-5 py-4">
          <div className="flex flex-col items-center text-center">
            ...
            <div className="flex items-center justify-center gap-2 mt-1.5 text-base font-semibold text-gray-300">
              <span className="text-sky-300">{predictedRecord.display}</span>
              <span>predicted</span>
              {playoffOdds != null && (
                <>
                  <span>•</span>
              ...
              <span>•</span>
              <span>
                {counts.forwards}F / {counts.defensemen}D / {counts.goalies}G
              </span>
            </div>
          </div>
        </div>
          ...
          {similarTeams && similarTeams.length > 0 && (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2.5 place-items-center">
                {similarTeams.slice(0, 4).map((similar) => (
                  <div
                  ...
                  >
                    <div className="relative w-16 h-16 mx-auto mb-1">
                      <div
                        ...
                        style={{
                          border: `1px solid ${playerUtils.toRgba(playerUtils.getTeamColor(similar.team, similar.season, "dark"), 0.3)}`,
                        }}
                      >
                        <img
                          src={playerUtils.getCorsWrappedUrl(
